...
- **Новый тест `_test-online-game.js`** — host+guest через реальную net.js, агент дёргает `ui.tm.*` (на guest это прокси в сеть). Проверяет sync после каждого хода, доходит до game-over за ~30 ходов. Написан взамен хрупкого `_test-e2e-full.js` (оставлен рядом, но заменён в основном workflow). 2p normal: T32 winner=1 [4,15]. 2p hard: T34 winner=1 [3,15]. 0 десинхов за обе партии.
- **Фикс `_test-e2e-full.js`** — добавлен вызов `tm.replenish()` в начале хода агента: без него host никогда не переходил в Turn-фазу и за всю партию не играл ни одной карты (было `host:0 / guest:12`).
...
- Root cause layout: `#app` = `height: 100dvh` + 9 grid-rows. `revealed-wrap` (88px) и `card-desc-bot` (80px, placeholder) зарезервированы ВСЕГДА — даже когда раскрытых карт нет и карта не выбрана. На iPhone SE с URL-баром (~553px dvh) доска получала ~162px и не умещалась в 4×4.
- Root cause «Снова»: `btn-play-again.onclick = _startGame(playerCount)` — без netOpts → `netMode` сбрасывался в null → локальный hot-seat.
- Фикс layout: при пустой зоне раскрытых добавляю `.collapsed` → `display: none`. Placeholder у `card-desc-bot` коллапсирую в `display: none` — до выбора карты занимает 0px. Освобождает 168px доске. При выборе карты desc-bot появляется (80px, доска сжимается, но не клипается).
- Фикс «Снова»: скрываю `btn-play-again` когда `this.netMode` не null. В онлайне игрок возвращается в меню и подключается заново.
- Проверка: playwright на viewport 375×553 — доска 162×162 → 330×330 в idle, 250×250 при выбранной карте. Умещается полностью.

---

## 2026-04-20 — Online фикс: guest разыгрывает карту, фишки не снимаются с поля

- Баг: когда guest (клиент) разыгрывал карту с паттерном, фишки паттерна не снимались с поля, очки не начислялись. Для host (хоста) всё работало корректно.
- Root cause: `_hostHandleAction` [ui.js:2398](Web/ui.js:2398) искал карту через `this.cardsById.get(id)` — отдельный Map с НОВЫМИ объектами карт (`buildCardsById()` вызывает `CardDatabase.create()` повторно, создавая новые экземпляры). Но валидация в `tm.playCard` [game.js:481](Web/game.js:481) использует `pl.hand.includes(card)` (сравнение по ссылке). Карта из `cardsById` ≠ карта в `pl.hand` → `owned=false` → `invalidAction` → тихий reject.
- Фикс: `_hostHandleAction` теперь ищет карту по id внутри **реального** state хоста — по рукам всех игроков, раскрытым, сбросу, колоде. Одинаковая ссылка на объект гарантирует что `pl.hand.includes()` вернёт true.
- Применяется к `playCard`, `utilizeCard`, `synthesis`.
- Проверка: playwright-тест с двумя браузерами. ДО фикса — guest играет ДУБЛИРОВАНИЕ с паттерном на (1,0),(1,2), после розыгрыша чипы на (1,0) и (1,2) остаются, счёт не меняется. ПОСЛЕ фикса — чипы снимаются, счёт +1.

---

## 2026-04-20 — PlaceChips: превью фишки на выбранных клетках во время ввода

- До: в эффектах `PlaceChipsEffect` (и других с node pick) при каждом промежуточном тапе клетка получала только класс `.selected-node { opacity: 0.45 }`. Поскольку исходный класс клетки оставался `empty` (width 8% у `::before`), реальная фишка цвета игрока не отрисовывалась — был виден лишь чуть потускневший фоновый ромбик. Пользователь не видел где уже тапнул, пока не поставил **все** N фишек (после финализации `_renderBoard` ставил `p1`/`p2`/`p3` и ромб «проявлялся»).
- Фикс:
  - `_renderBoard` [ui.js:656](Web/ui.js:656) — для клеток из `selectedSet` и с `occ === 0` базовый класс подменяется с `empty` на `p1`/`p2`/`p3` (текущий игрок). Тогда `.node.p{N}::before` рисует крупный ромб 78% нужного цвета поверх фона, а `.selected-node { opacity: 0.45 }` делает его полупрозрачным — получается чёткое «призрачное» превью.
  - `_handleNodePick` [ui.js:~1592](Web/ui.js) — после каждого валидного тапа вызывает `_renderBoard()`, чтобы превью появилось сразу, а не после финальной финализации эффекта.
- Проверка: в preview симулирован промежуточный pick (3 клетки выбраны, эффект ещё активен) — `getComputedStyle(cell, '::before')` показал `width: 68.25px` (78% от 87.5), `background: rgb(63,208,230)` (= `--p1`), cell `opacity: 0.45`. Скриншот: крупные голубые полупрозрачные ромбы на (0,0), (0,3), (2,1).
- Регрессия: scenarios 13/13, fuzz 1000/2p = 0 крашей, online-steal ✓, online-reconnect ✓, visual-phones 7/7.

---

## 2026-04-20 — Фишки паттерна снимаются ДО эффекта розыгрыша/синтеза

- До: в `playCard` (game.js:487) порядок был «+очки → эффект → снять фишки паттерна → карта в сброс». При розыгрыше карты с эффектом «поставить N фишек» клетки только что задействованного паттерна были заняты (фишки ещё не сняты) → пользователь не мог поставить туда новые фишки, хотя по интуиции настольной игры эти клетки уже свободны после розыгрыша. Аналогично в `synthesis` — объединение фишек обеих карт снималось в `finalize` (после обоих эффектов).
- Фикс:
  - `playCard` [game.js:487](Web/game.js:487) теперь снимает фишки паттерна **до** `playEffect.execute`. Порядок: «+очки → снять фишки паттерна → эффект → карта в сброс».
  - `synthesis` [game.js:563](Web/game.js:563) снимает объединение фишек обеих карт (Set-дедуп по общим позициям) **до** эффектов. Валидация `hasShared` и `findMatches` остались на прежнем месте — проверяются до снятия.
- CLAUDE.md обновлён — правило и раздел «Синтез» переформулированы, зафиксирована новая семантика.
- Сценарии `_test-scenarios.js`: добавлен «Сценарий 4 — PlaceChips видит клетки паттерна как пустые». Покрывает playCard (МИГРАЦИЯ, Place(1), доска забита P2 кроме (1,1) — эффект должен ставить фишку обратно в (1,1)) и synthesis (МИГРАЦИЯ+БИТ, оба W(1,1) overlap). Оба PASS.
- Регрессия: fuzz 1000/2p = 0 крашей, scenarios 13/13, online-steal ✓, online-reconnect ✓. В preview: `empty=["1,1"]`, карта сыграна, фишка P1 снова встала в (1,1).

---

## 2026-04-20 — PHASE-подсказка: обрезка на ультра-узких экранах (Galaxy-Fold 280px)

- До: `#phase-hint` имел фиксированный `height: 32px` + `overflow: hidden`. На Galaxy-Fold (280×653) текст "СТАВЬ фишки · розыгрыш невозможен" не помещался в одну строку → переносился на две → `scrollHeight: 35px`, но контейнер 32px обрезал низ — было видно "невозмож_", хвост срезан.
- Фикс: медиа-запрос `@media (max-width: 320px)` в [index.html:2312](Web/index.html:2312) снимает фиксированную высоту и даёт контейнеру растянуться: `height: auto; min-height: 32px; padding-top/bottom: 3px; align-items: flex-start; overflow: visible`. `line-height: 1.3` для `.hint-text`. Grid-row у `#app` — `auto`, поэтому доска просто подвинется на 5px вниз, без конфликтов.
- Проверка: после фикса на 280×653 — `hintHeight: 37, clipped: false`, обе строки "СТАВЬ фишки · розыгрыш / невозможен" видны полностью. На 390×844 — `hintHeight: 32, clipped: false` (без изменений). Visual-phones 7/7 (iPhone-SE / 14 / 14-Pro-Max / Pixel-5 / Galaxy-S20 / Galaxy-Fold / iPhone-5).

---

## 2026-04-20 — Анимация ухода карты с руки при розыгрыше/утилизации/синтезе

- До: при розыгрыше / утилизации / синтезе карта просто исчезала из руки после перерисовки `render()`. Никакой визуальной обратной связи — пользователь не всегда понимал, какая карта ушла в сброс, особенно в синтезе с двумя картами одновременно.
- Фикс: добавлена анимация "fly-out" — клон карты на `position: fixed` с `z-index: 500` вылетает вверх с параллаксом к центру экрана (учёт `--fly-dx` для горизонтального дрейфа к центру, чтобы карты с краёв руки сходились), с `scale(1.08)` → `scale(0.55)` и фейдом. Ключевые кадры в [index.html:1114](Web/index.html:1114) — `@keyframes cardFlyOut` (0.55s, cubic-bezier).
- Реализация: `_makeCardEl` в [ui.js:889](Web/ui.js:889) теперь пишет `el.dataset.cardId = card.id`. Новый метод `_animateCardFlyOut(cardId)` клонирует карту, фиксирует её позицию через `getBoundingClientRect()`, навешивает класс `.card-flying-out` и скрывает оригинал (`visibility: hidden`). Вызывается до `tm.playCard` ([ui.js:1541](Web/ui.js:1541)), до `tm.utilizeCard` ([ui.js:1373](Web/ui.js:1373)) и для обеих карт синтеза ([ui.js:1416](Web/ui.js:1416)). Клон удаляется через 600мс.
- Проверка: в preview — клон корректно создан (`position: fixed`, `z-index: 500`, `animation: cardFlyOut`, оригинал скрыт). Регрессия: fuzz 1000/2p = 0 крашей, scenarios 11/11, online-steal ✓, online-reconnect ✓, visual-phones 7/7.

---

## 2026-04-20 — Онлайн: автореконнект гостя при обрыве связи

- До: когда у гостя обрывалось WebRTC-соединение (tab suspend на мобилке, смена сети, потеря пакетов), overlay показывал «Пытаемся переподключиться…», но на деле `reconnectGuest()` не вызывался ниоткуда — оставался только мёртвый conn и ручной выход в меню. Плюс если conn формально «открыт», но канал мёртв (без ping/pong), хост/гость продолжали висеть не зная об этом.
- Фикс 1 — watchdog: в `_startKeepalive()` каждые 5с, если `Date.now() - _lastPingTs > 15000`, принудительно закрываем conn → триггерится `conn.on('close')` → `onDisconnect`. Раньше `_lastPingTs` обновлялся, но никто не проверял его.
- Фикс 2 — автореконнект: новый метод `Net.startReconnectLoop(onAttempt, onGiveUp, maxAttempts=20)` запускает `reconnectGuest()` каждые 3с. `reconnectGuest()` делает новый `peer.connect(sameCode)` и биндит его с флагом `isReconnect=true` → хост в `on('connection')` закрывает старый conn и принимает новый → отправляет свежий snapshot по `hello`. После 20 попыток (~1 мин) — `onGiveUp` меняет overlay на «Не удалось переподключиться».
- UI: в [ui.js:2271](Web/ui.js:2271) `onDisconnect` теперь вызывает `startReconnectLoop` с колбэками — показывает прогресс `(N/20)`, по провалу меняет заголовок. Кнопка «Выйти в меню» уже была.
- Тест `_test-online-reconnect.js`: после старта игры вручную закрывает `net.conn` у гостя, ждёт появления overlay, ждёт `net.connected === true` (автореконнект ≤15с), проверяет что state (currentPI, phase, deckCount, scores, handLen) идентичен до/после и что host тоже считает себя connected. PASS.
